	...
	private String getResponseBody(String url) {
		...
		try {
			...
			HttpResponse<JsonNode> request = Unirest.get(url)
					.header("apisign", EncryptionUtility.calculateHash(secret, url, "HmacSHA512"))
					.asJson();

			Gson gson = new GsonBuilder().setPrettyPrinting().create();
			responseJson = gson.toJson(request.getBody().getObject()); // Cuts what the server replies to only the message, the success & the actual response
